...
def get_shengfukexie(bazi,tiangan_dizhi):
...
def get_yueling_score(bazi,tiangan_dizhi):
    rizhu = bazi[0][2]
    yueling = bazi[1][1]
...
def get_bazi_score(wuxing_score,bazi,tiangan_dizhi):
    # 计算八字位置所得分数
    bazi_wuxing = get_shengfukexie(bazi,tiangan_dizhi)
...
def main(arg1):
        ...
        # 计算月令五行得分
        yueling_wuxing,yueling_score = get_yueling_score(bazi,tiangan_dizhi)
        ...
        bazi_sfzk_list = []
        for item in bazi_sfzk.keys():
            bazi_sfzk_list.append(item+":"+bazi_sfzk[item])
